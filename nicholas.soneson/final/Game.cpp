...
void Game::createGraphicsBuffers()
{
	...
	string audioPath = Game::getInstance()->getDataLoader()->getAudioPath();
	string shotSound = Game::getInstance()->getDataLoader()->getShotSoundFile();
	mpSystem->getSoundSystem()->setSoundBuffer(audioPath + shotSound);
}
...
void Game::createFirstLevel()
{
	...
	string firstLevel = Game::getInstance()->getDataLoader()->getFirstLevel();
	mpCurrentLevel->initLevelData(firstLevel);
}
		...
		if (theEvent.getType() == MENU_SELECT_EVENT)
		{
			...
			else
			{
				...
				else if (currentOptionsSelection == MENU_OPTIONS_SELECT_LANGUAGE)
				{
					...
					string langPath = Game::getInstance()->getDataLoader()->getLanguagePath();

					if (mpMenu->getCurrentLanguage() == 0)
					{
						//lang one
						string englishFile = Game::getInstance()->getDataLoader()->getEnglishLangFile();
						mpLanguageSelect->loadLanguageFromFile(langPath + englishFile);
					...
					{
						//lang two
						string spanishFile = Game::getInstance()->getDataLoader()->getSpanishoLangFile();
						mpLanguageSelect->loadLanguageFromFile(langPath + spanishFile);
						mpMenu->reloadLanguage();
					...
					{
						//lang three
						string tLangFile = Game::getInstance()->getDataLoader()->getTLangFile();
						mpLanguageSelect->loadLanguageFromFile(langPath + tLangFile);
						mpMenu->reloadLanguage();
